...
for (E_Int noind = 0; noind < ifin-ideb; noind++)
{
  ...
  if (yplusext_vec[noind] > 0.)
  {
    /*
      Resolution du systeme suivant avec formules de Cramer:
        nutext_vec - kappa*yplusext_vec  = alpha2*yplusext_vec*yplusext_vec + alpha3*yplusext_vec*yplusext_vec*yplusext_vec
        (nu/utau)*gradNutext_vec - kappa = alpha2*2*yplusext_vec            + alpha3*3*yplusext_vec*yplusext_vec
    */

    yplusext_vec[noind] = yplusext_vec[noind]*utau_vec[noind];
    E_Float nu = (mu_vec[noind]/ro_vec[noind]);
    
    E_Float a = yplusext_vec[noind]*yplusext_vec[noind];
    E_Float b = yplusext_vec[noind]*yplusext_vec[noind]*yplusext_vec[noind];
    E_Float c = nutext_vec[noind] - kappa*yplusext_vec[noind];

    E_Float ap = 2*yplusext_vec[noind];
    E_Float bp = 3*yplusext_vec[noind]*yplusext_vec[noind];
    E_Float cp = (nu/utau_vec[noind])*gradNutext_vec[noind] - kappa;

    E_Float det = a*bp - ap*b;

    E_Float alpha2 = (c*bp - cp*b)/det;
    E_Float alpha3 = (a*cp - ap*c)/det;

    if (gradP_vec[noind] > 0.) nutcible_vec[noind] = kappa*yplus_vec[noind] + alpha2*yplus_vec[noind]*yplus_vec[noind] + alpha3*yplus_vec[noind]*yplus_vec[noind]*yplus_vec[noind];
    else nutcible_vec[noind] = kappa*yplus_vec[noind];
    
    aa_vec[noind] = nutcible_vec[noind]*nu;
    // printf("yplusext_vec = %f / yplus_vec = %f / gradNutext_vec = %f / nutext_vec = %f / nutcible_vec = %f\n", yplusext_vec[noind], yplus_vec[noind], gradNutext_vec[noind]*nu, nutext_vec[noind]*nu, nutcible_vec[noind]*nu);
  }
  else
  {
    // van driest pour nut
